...
{
    public static void mergeSort(Song[] a, int from, int to) {
        if (from == to) {
            return;
        }
        int mid = (from + to) / 2;
    ...
    public static void merge(Song[] a, int from, int mid, int to) {
        int n = to - from + 1;       // size of the range to be merged
        Song[] b = new Song[n];   // merge both halves into a temporary array b
